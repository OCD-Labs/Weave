...
**Status:** Pre-implementation. This document is the complete contract, backend, and integration architecture for Weave. No implementation code has been written yet.
...
contract WeaveRegistry {
    ...
    address public usdc;                    // canonical USDC on Robinhood Chain
    ...
    uint256 public minFirstDepositUsdc;     // prevents first-depositor manipulation
...
// First depositor receives tokens at a fixed 1 USDC : 1 token ratio.
function deposit(
    uint256 usdcAmount,
...
// Distributes fee USDC to protocol treasury and creator token revenue pool.
function collectFees() external nonReentrant;

// View: current total basket value in USDC (sum of constituent balances * Chainlink prices)
function totalValueUsdc() external view returns (uint256);

// View: current value of one basket token in USDC
...
The management fee is charged as a percentage of USDC on each deposit and each redemption rather than as a time-based AUM dilution. This avoids the complexity of minting new basket tokens over time and keeps the fee logic transparent and predictable.

On every deposit:

```
fee_usdc = deposit_usdc * managementFeeBps / 10_000
net_deposit_usdc = deposit_usdc - fee_usdc
protocol_cut = fee_usdc * protocolShareBps / 10_000
creator_cut = fee_usdc - protocol_cut

transfer protocol_cut → registry.protocolTreasury
call creatorToken.snapshotRevenue(creator_cut) with USDC transfer
buy constituents using net_deposit_usdc only
mint basket tokens based on net_deposit_usdc contribution
```

On every redemption:

```
gross_usdc_value = basket_tokens_being_redeemed * navPerToken
sell constituents proportional to gross_usdc_value
fee_usdc = gross_usdc_value * managementFeeBps / 10_000
net_usdc = gross_usdc_value - fee_usdc
protocol_cut = fee_usdc * protocolShareBps / 10_000
creator_cut = fee_usdc - protocol_cut

transfer protocol_cut → registry.protocolTreasury
call creatorToken.snapshotRevenue(creator_cut) with USDC transfer
transfer net_usdc → receiver
burn basket tokens
```
...
```
totalValueUsdc = sum over all constituents:
    constituentBalances[i] * chainlinkPrice(constituents[i]) / 1e8
```

Chainlink prices for stock feeds are 8-decimal. Constituent balances are 18-decimal (standard ERC-20). The result is in USDC 6-decimal terms. All intermediate multiplications use `uint256` with no overflow risk at realistic portfolio sizes.

**NAV per token:**

```
navPerToken = totalValueUsdc * 1e18 / basketToken.totalSupply()
```

Expressed in USDC per basket token with 18 decimal precision.

**Basket tokens minted on deposit:**

```
// After fee deduction, net_deposit_usdc is available to invest.
// If total supply is zero (first depositor):
basket_tokens_minted = net_deposit_usdc * 1e12
// (converts 6-decimal USDC to 18-decimal basket token, establishing 1:1 initial price)

// If total supply is non-zero:
basket_tokens_minted = net_deposit_usdc * 1e12 * totalSupply / totalValueUsdc
```

The first depositor establishes a 1:1 ratio of 1 USDC to 1 basket token. All subsequent depositors are priced against the current NAV, so their share of the pool is proportional to their contribution relative to the existing pool value. There is no first-depositor donation attack risk here because the basket immediately deploys deposits into constituent purchases rather than holding USDC, so directly transferring constituent tokens to the basket without going through `deposit()` would change the basket value but all existing holders benefit proportionally from that donation. A malicious actor gains nothing from it.

**Constituent purchase proportions on deposit:**

```
// For each constituent i:
usdc_to_spend_on_i = net_deposit_usdc * targetWeightsBps[i] / 10_000
constituent_tokens_received = swapRouter.swapExactUSDCForToken(
    constituents[i],
    usdc_to_spend_on_i,
...
currentWeightBps[i] = value_i * 10_000 / totalValueUsdc
...
target_value_i = totalValueUsdc * targetWeightsBps[i] / 10_000
delta_i = current_value_i - target_value_i

// Positive delta: overweight → sell delta_i USDC worth of constituent i
// Negative delta: underweight → buy |delta_i| USDC worth of constituent i

// Sell all overweight positions first, accumulate USDC
// Then use accumulated USDC to buy all underweight positions
// This avoids needing to know trade ordering in advance
```

All rounding truncates toward zero. Residual USDC from rounding differences stays in the basket and is distributed proportionally on the next redemption.

---

## 9. CreatorToken (ERC-7641)

One creator token contract is deployed per basket. The creator token is an ERC-20 with a fixed initial supply of 1,000,000 units (18 decimals), all minted to the basket creator at deployment. It implements ERC-7641 to enable a revenue sharing claim mechanism against a USDC revenue pool.
...
contract CreatorToken is ERC20, IERC7641 {
    ...
    // snapshotId → total USDC added to pool at that snapshot
    ...
    constructor(address _basket, address _usdc, address _creator) {
        basket = _basket;
        usdc = _usdc;
    ...
    // Receives USDC from the basket and records a new snapshot.
    // Only callable by the associated basket contract.
    function snapshotRevenue(uint256 usdcAmount) external onlyBasket;

    // ERC-7641: returns USDC claimable by account for a given snapshot
    function claimableRevenue(address account, uint256 snapshotId)
        external view returns (uint256);

    // ERC-7641: claims USDC for a given snapshot
...
interface IWeaveRouter {
    // Swap exact USDC in for at least minTokenOut of token
    ...
    // Swap exact tokenIn for at least minUsdcOut USDC
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        for (uint256 i = 0; i < allBaskets.length && count < batchSize; i++) {
            ...
            if (
                ...
                !basket.suspended() &&
                basket.totalValueUsdc() >= IWeaveRegistry(registry).minAUMForAutomation() &&
...
**Deposit when DEX has insufficient liquidity for one constituent.** The DEX router call reverts. The entire deposit transaction reverts. The user's USDC is returned. The basket state is unchanged. This is the correct safe failure mode. The user should retry when liquidity improves or reduce their deposit size to stay within available liquidity depth.

**Rebalancing trades result in dust.** After a rebalancing sequence, rounding means the basket may hold tiny USDC residuals that could not be fully deployed into constituent positions. These residuals accumulate in the basket contract as USDC. On the next deposit, this USDC is included in the basket's constituent purchase calculation. It is never stuck.
...
**First depositor minimum not met.** The factory reverts if `initialDepositUsdc < registry.minFirstDepositUsdc`. The creator must fund the basket above the minimum at launch. This ensures the basket has a meaningful NAV from the start and prevents economically trivial baskets from being listed.
...
**Stock split where Robinhood adjusts token supply.** If Robinhood doubles the token supply in a 2:1 split, the basket's constituent balance doubles automatically (since the basket holds the tokens and split credits all holders). The Chainlink price halves simultaneously to reflect the split. The product `balance * price` remains constant, so `totalValueUsdc` and all NAV calculations are unaffected. No special handling is needed.
...
**Fee collection when basket has zero AUM.** If a basket has no deposits, `totalValueUsdc = 0` and `fee_usdc = 0`. No USDC moves. No snapshot is created on the creator token. This is correct and requires no special handling.
...
CREATE TABLE nav_history (
    ...
    total_value_usdc TEXT NOT NULL,
    timestamp INTEGER NOT NULL
);
```
...
Deposited(address indexed basket, address indexed investor, uint256 usdcAmount, uint256 basketTokensMinted, uint256 feeUsdc)
Redeemed(address indexed basket, address indexed investor, uint256 basketTokensBurned, uint256 usdcReturned, uint256 feeUsdc)
Rebalanced(address indexed basket, address indexed triggeredBy)
FeeSnapshot(address indexed basket, uint256 snapshotId, uint256 usdcAmount)
...
GET /baskets/:address/positions/:wallet
  Returns the investor's current basket token balance, current USDC value, entry cost basis
...
2. Go backend fetches the full catalogue from its own database including symbol, name, sector, and current price for every active asset
3. Go backend forwards the request to the TypeScript AI service with the thesis and catalogue data
4. TypeScript service constructs a prompt for the Anthropic API
5. Anthropic API returns a JSON proposal
...
**Chainlink Data Feeds.** Every supported stock has a Chainlink price feed address stored in `registry.assets[token].chainlinkFeed`. All price reads call `latestRoundData()` which returns `(roundId, answer, startedAt, updatedAt, answeredInRound)`. The `answer` field is the price in 8-decimal USD. The `updatedAt` field is validated against `block.timestamp - registry.oracleStalenessSecs` before any value is used. Chainlink is confirmed as Robinhood Chain's official oracle partner and provides 24/5 feeds for tokenized US stocks.

**Alchemy RPC.** Alchemy is a confirmed infrastructure partner on Robinhood Chain testnet. The Go backend uses Alchemy's Robinhood Chain RPC endpoint for all event subscription and state reads. The basket contracts interact with on-chain state directly, no backend dependency.

**Anthropic API.** The AI composition engine calls `claude-sonnet-4-20250514` via the Anthropic messages API. The prompt is structured to produce JSON output deterministically. The engine does not stream responses and waits for the full completion before validating. API key is stored as an environment variable and never exposed to the frontend.

**Chainlink Automation.** The WeaveAutomation contract is registered as a custom logic upkeep on Chainlink Automation. The upkeep is funded with LINK from the protocol treasury. The protocol monitors the LINK balance of the upkeep and tops it up when it falls below a threshold. Chainlink's docs confirm that custom logic upkeeps support `checkData` for parameterising which baskets each job monitors, enabling pagination across multiple jobs as the basket count grows.
...
```
1. WeaveRegistry(governance, usdc, protocolTreasury, managementFeeBps, ...)
...
**Reentrancy.** Every state-mutating basket function uses OpenZeppelin's `ReentrancyGuard`. All external calls are last in the execution sequence. The ERC-7641 `claim()` function also uses a reentrancy guard since it sends USDC to the caller.
...
**DEX availability on Robinhood Chain testnet.** The MockSwapRouter handles this for the submission but the mainnet DEX that will have meaningful liquidity for tokenized stocks is not yet confirmed. This will be resolved by the mainnet launch timing and ecosystem development.

**Chainlink feed addresses on Robinhood Chain testnet.** Chainlink is a confirmed infrastructure partner but specific feed addresses for Robinhood Chain testnet are not yet publicly documented. These will be obtained from Chainlink's documentation or from the June 2 Adam Fern workshop.

**USDC availability on Robinhood Chain testnet.** Circle's native USDC via CCTP is the target. If only bridged USDC.e is available on testnet, the registry's `usdc` address is set to USDC.e and updated to native USDC on mainnet with no contract changes required.
